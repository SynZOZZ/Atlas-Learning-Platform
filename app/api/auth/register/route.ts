...
export async function POST(request: Request) {
  try {
    ...
    const user = await one<{ id: number; email: string; role: "student" }>(
      `INSERT INTO users (email,password_hash,name,phone,whatsapp,country,city,specialty,role,status,trusted_device_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'student','pending',$9) RETURNING id,email,role`,
      [email, passwordHash, name, phone, clean(body.whatsapp, 30), clean(body.country, 80), clean(body.city, 80), clean(body.specialty, 160), deviceId],
    );
